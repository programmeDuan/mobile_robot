#include "../include/calib_odom/laser_scan.h"

YATOdomCalib Odom_calib;

void YATScan:: yatPublishPath( Eigen::Vector3d& pose,nav_msgs::Path &path,ros::Publisher &mcu_path_pub_)
{
...
void YATScan::yatPublishPathEigen(std::vector<Eigen::Vector3d>& path_eigen,ros::Publisher& path_pub_)
{
...
*/
bool YATScan::yatGetOdomPose(Eigen::Vector3d& pose, const ros::Time& t)
{
    ...
    try
    {
        tf_.transformPose(odom_frame_, ident, odom_pose);//ident源pose数据,odom_pose目标pose数据
    }
    ...
    pose << odom_pose.getOrigin().x(),
            ...
            yaw;
    //yatPublishPath(pose,path_odom,odom_path_pub_);
    return true;
}
//构造函数
YATScan::YATScan()
{
    ...
    calib_flag_sub_ = node_.subscribe("calib_flag",5,&YATScan::yatCalibFlagCallBack,this);
    ...
    scan_filter_sub_ = new message_filters::Subscriber<sensor_msgs::LaserScan>(node_, "/scan", 10);
    scan_filter_ = new tf::MessageFilter<sensor_msgs::LaserScan>(*scan_filter_sub_, tf_, odom_frame_, 10);
    scan_filter_->registerCallback(boost::bind(&YATScan::yatScanCallback, this, _1));

    std::cout <<"Calibration Online,Wait for Data!!!!!!!"<<std::endl;
}

//订阅topic表示开始进行标定.
void YATScan::yatCalibFlagCallBack(const std_msgs::Empty &msg)
{
    Eigen::Matrix3d correct_matrix = Odom_calib.yatSolve();
    ...
    //发布矫正之后的路径
    yatPublishPathEigen(calib_path_eigen,calib_path_pub_);
...
//激光数据回调函数
void YATScan::yatScanCallback(const sensor_msgs::LaserScan::ConstPtr &_laserScanMsg)
{
    ...
    //得到对应的里程计数据
    if(!yatGetOdomPose(odom_pose, _laserScanMsg->header.stamp))
        return ;

    //前后两帧里程计的位姿差
    d_point_odom = yatCalDeltaDistance(odom_pose);
    ...
    if(m_prevLDP != NULL)
    {
        yatLaserScanToLDP(&scan,currentLDP);
        d_point_scan = yatPIICPBetweenTwoFrames(currentLDP,d_point_odom);
    }
    else
    {
        yatLaserScanToLDP(&scan,m_prevLDP);
    }
    ...
    yatPublishPath(odom_pos_cal,path_odom,odom_path_pub_);
    yatPublishPath(scan_pos_cal,path_scan,scan_path_pub_);

    //构造超定方程组
    Odom_calib.yatAddData(d_point_odom,d_point_scan);
    //求解最小二乘方程组//
    Eigen::Matrix3d correct_matrix = Odom_calib.yatSolve();
...
Eigen::Vector3d YATScan::yatCalDeltaDistance(Eigen::Vector3d odom_pose)
{
    ...
    //TODO:
    Eigen::Matrix3d tmp_transform_matrix;
    tmp_transform_matrix << cos(last_pos(2)),-sin(last_pos(2)),0,
                            sin(last_pos(2)), cos(last_pos(2)),0,
                                           0,                0,1;
    d_pos = tmp_transform_matrix.transpose()*(now_pos-last_pos);
    //end of TODO:
...
//设置PI-ICP的参数
void YATScan::yatSetPIICPParams()
{
    //设置激光的范围//
...
void YATScan::yatLaserScanToLDP(sensor_msgs::LaserScan *pScan,LDP& ldp)
{
...
//求两帧之间的icp位姿匹配//
Eigen::Vector3d  YATScan::yatPIICPBetweenTwoFrames(LDP& currentLDPScan,
